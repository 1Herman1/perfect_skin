...
| GET | `/api/v1/orders/track` | Проверить заказ по номеру и email (для гостей) | нет |
...
### 3.17a `GET /api/v1/orders/track`

Публичная проверка заказа по номеру и email **без авторизации**. Гость может отследить статус заказа, не входя в аккаунт.

**Параметры запроса:**

| Параметр | Валидация | Обязательность |
|---|---|---|
| `number` | `^PS-\d{6}$` | всегда |
| `email` | email, до 254 символов | всегда |

Email нормализуется: `trim().toLowerCase()`. Сравнение с email получателя заказа проводится в нижнем регистре.

**Ответ 200** — тот же формат, что `GET /orders/:number` для авторизованного пользователя (тот же `$ref: 'ps.order#'`).

**Безопасность:** если заказ не найден **или** email не совпадает с адресом получателя, сервер возвращает единый ответ `404 ORDER_NOT_FOUND`. Никогда не раскрывает, существует ли номер — это защита от перебора номеров (номер короткий: всего 1 млн комбинаций).

**Rate limit:** 20 запросов / 15 минут по IP.

**Ошибки:** `400 VALIDATION_ERROR` (кривой формат number или email), `404 ORDER_NOT_FOUND` (заказ не найден или email не совпадает), `429 RATE_LIMITED`.
